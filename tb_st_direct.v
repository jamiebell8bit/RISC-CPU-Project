...
module tb_st_direct;
    ...
    initial begin
        ...
        // Case: st 0x1F, R6 ; R6=0x63 and (0x1F)=0xD4 -> expect mem[0x1F]=0x63
        DUT.R6.q = 32'h00000063;
        DUT.RAM0.mem[9'h01F] = 32'h000000D4;
        DUT.PC.q = 32'h00000000;
        fetch_instr({5'b10010, 4'd6, 4'd0, 19'h0001F});

        // T3  compute address base + C
        ...
        // T5
        Zlowout = 1'b1; MARin = 1'b1;
        tick();

        // T6  move source register into MDR
        Gra = 1'b1; Rout = 16'h0001; MDRin = 1'b1;
        tick();
        ...
        $display("st_direct: mem[1F]=%h expected 00000063", DUT.RAM0.mem[9'h01F]);
        if (DUT.RAM0.mem[9'h01F] !== 32'h00000063) $display("FAIL: st direct");
        else $display("PASS: st direct");
        #20; $stop;
    end
endmodule
