...
module tb_andi;
    ...
    initial begin
        ...
        // andi R7, R4, 0x71 ; choose R4 = 0xF3
        DUT.R4.q = 32'h000000F3;
        DUT.PC.q = 32'h00000000;
        fetch_instr({5'b01010, 4'd7, 4'd4, 19'h00071});

        Grb = 1'b1; Rout = 16'h0001; Yin = 1'b1;
        tick();

        Cout = 1'b1; opcode = 5'b00010; Zin = 1'b1;
        tick();

        Gra = 1'b1; Zlowout = 1'b1; Rin = 16'h0001;
        tick();

        $display("andi: R7=%h expected 00000071", DUT.R7.q);
        if (DUT.R7.q !== 32'h00000071) $display("FAIL: andi");
        else $display("PASS: andi");
        #20; $stop;
    end
endmodule
