...
module tb_mfhi_mflo;
    ...
    initial begin
        ...
        DUT.PC.q = 32'h00000000;
        fetch_instr({5'b11000, 4'd5, 23'd0});
        Gra = 1'b1; HIout = 1'b1; Rin = 16'h0001;
        tick();
        $display("mfhi: R5=%h expected 12345678", DUT.R5.q);
        if (DUT.R5.q !== 32'h12345678) $display("FAIL: mfhi");
        else $display("PASS: mfhi");

        // mflo R1
        zero_state();
        DUT.LO.q = 32'h89ABCDEF;
        DUT.PC.q = 32'h00000000;
        fetch_instr({5'b11001, 4'd1, 23'd0});
        Gra = 1'b1; LOout = 1'b1; Rin = 16'h0001;
        tick();
        $display("mflo: R1=%h expected 89ABCDEF", DUT.R1.q);
        if (DUT.R1.q !== 32'h89ABCDEF) $display("FAIL: mflo");
        else $display("PASS: mflo");
        #20; $stop;
    end
endmodule
