...
module tb_jr_jal;
    ...
    initial begin
        ...
        fetch_instr({5'b10100, 4'd12, 23'd0});

        Gra = 1'b1; Rout = 16'h0001; PCin = 1'b1;
        tick();

        $display("jr: PC=%h expected 000000FF", DUT.PC.q);
        if (DUT.PC.q !== 32'h000000FF) $display("FAIL: jr");
        else $display("PASS: jr");

        // -----------------------------
        // jal R4
        // Workaround for current repo decode:
        // temporarily point Gra to R12 so current PC can be saved.
        // -----------------------------
        zero_state();
        DUT.R4.q = 32'h000000AA;
        DUT.PC.q = 32'h00000010;
        fetch_instr({5'b10011, 4'd4, 23'd0});

        // Save return address (current PC already equals old PC+1 after fetch)
        DUT.IR0.q[26:23] = 4'd12;
        PCout = 1'b1; Gra = 1'b1; Rin = 16'h0001;
        tick();

        // Restore jal destination and jump to R4
        DUT.IR0.q[26:23] = 4'd4;
        Gra = 1'b1; Rout = 16'h0001; PCin = 1'b1;
        tick();

        $display("jal: R12=%h expected 00000011, PC=%h expected 000000AA", DUT.R12.q, DUT.PC.q);
        if (DUT.R12.q !== 32'h00000011 || DUT.PC.q !== 32'h000000AA) $display("FAIL: jal");
