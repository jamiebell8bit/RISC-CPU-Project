...
module tb_ld_direct;
    ...
    initial begin
        ...
        // Case: ld R7, 0x65   with (0x65) = 0x84
        DUT.RAM0.mem[9'h000] = 32'h00000000;
        DUT.RAM0.mem[9'h065] = 32'h00000084;
        DUT.PC.q = 32'h00000000;

        fetch_instr({5'b10000, 4'd7, 4'd0, 19'h00065});
        ...
        // T4
        Cout = 1'b1; opcode = 5'b00000; Zin = 1'b1; // ADD
        tick();
        ...
        // T6
        Read = 1'b1; MDRin = 1'b1;
        tick();

        // T7
        Gra = 1'b1; MDRout = 1'b1; Rin = 16'h0001;
        tick();

        $display("ld_direct: R7=%h expected 00000084", DUT.R7.q);
        if (DUT.R7.q !== 32'h00000084) $display("FAIL: ld direct");
        else $display("PASS: ld direct");
        #20; $stop;
    end
endmodule
