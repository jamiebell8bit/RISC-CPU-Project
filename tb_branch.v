...
module tb_branch;
    ...
    begin
        ...
        DUT.R3.q = r3val;
        DUT.PC.q = start_pc;
        DUT.RAM0.mem[start_pc[8:0]] = {5'b10101, 4'd3, c2, 19'd48};
        fetch_instr({5'b10101, 4'd3, c2, 19'd48});

        // T3 evaluate condition into CON FF
        Gra = 1'b1; Rout = 16'h0001; CONin = 1'b1;
        tick();
        ...
        // T5 add sign-extended C
        Cout = 1'b1; opcode = 5'b00000; Zin = 1'b1;
        tick();

        // T6 CON -> PCin workaround using current repo output CON
        Zlowout = 1'b1; PCin = CON;
        tick();

        $display("%0s : CON=%b  PC=%h expected %h", name, CON, DUT.PC.q, expected_pc);
        if (DUT.PC.q !== expected_pc) $display("FAIL: %0s", name);
    ...
    initial begin
        clear_controls();
        #25;
        run_branch_case(4'b0000, 32'h00000000, 32'h00000010, 32'h00000041, "brzr taken");
        run_branch_case(4'b0001, 32'h00000000, 32'h00000010, 32'h00000011, "brnz not-taken");
        run_branch_case(4'b0010, 32'h00000005, 32'h00000010, 32'h00000041, "brpl taken");
        run_branch_case(4'b0011, 32'hFFFF_FFFF, 32'h00000010, 32'h00000041, "brmi taken");
        $stop;
    end
endmodule
